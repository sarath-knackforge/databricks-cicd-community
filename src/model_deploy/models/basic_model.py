...
class BasicModel:
    ...
    def model_improved(self) -> bool:
        ...
        Compares the current model with the latest registered model using F1-score.
        Returns True if:
        - No model exists yet (first run)
        - Current model performs better than old model
        """
        client = MlflowClient()

        # ----------------------------------------------------
        # 1️⃣ CHECK IF ANY MODEL EXISTS
        # ----------------------------------------------------
        try:
        ...
        if len(versions) == 0:
            logger.info("No existing model found — first run. Returning True.")
            return True

        # ----------------------------------------------------
        # 2️⃣ MODEL EXISTS → COMPARE F1 SCORES
        # ----------------------------------------------------
        try:
            ...
            old_eval = mlflow.models.evaluate(
                latest_model_uri,
                self.eval_data,
                targets=self.config.target,
                model_type="classifier",
            ...
            old_f1 = old_eval.metrics["f1_score"]

            # New model metrics
            new_f1 = self.metrics["f1_score"]

            logger.info(f"Old F1: {old_f1}, New F1: {new_f1}")

            return new_f1 >= old_f1

        except Exception as e:
            logger.warning(f"Could not evaluate old model, defaulting to improved=True. Error: {e}")
            return True
    ...
    def register_model(self) -> int:
        """Register model and set alias 'latest-model' (robust to registry differences)."""
        logger.info("🔄 Registering model...")

        client = MlflowClient()
        model_name = self.model_name  # e.g. "dev-model-deploy" or "mlops.models.model_deploy"

        model_uri = f"runs:/{self.run_id}/lightgbm-pipeline-model"

        # 1) Ensure registered model exists (create if missing)
        try:
            ...
            try:
                ...
                # Some registries may not require create_registered_model (it will be created by register_model)
                logger.warning(f"Could not create registered model explicitly: {e}")

        # 2) Register model version (this should create a version)
        try:
            registered_model = mlflow.register_model(model_uri=model_uri, name=model_name)
            logger.info(f"Model registered as version {registered_model.version}.")
        except Exception as e:
            # if register_model fails, bubble up with context
            logger.error(f"Failed to register model: {e}")
            raise

        # 3) Try to set alias 'latest-model' (some registries may not support aliases; handle gracefully)
        latest_version = registered_model.version
        try:
            client.set_registered_model_alias(name=model_name, alias="latest-model", version=latest_version)
            logger.info(f"Set alias 'latest-model' -> version {latest_version}")
        except Exception as e:
            logger.warning(f"Could not set alias 'latest-model' (may be unsupported). Continuing. Reason: {e}")
